...
- **LLM:** Nebius Token Factory API
- **You choose** which LLM model to use from the available models on Nebius Token Factory
...
1. The Nebius API key is configured via the **`NEBIUS_API_KEY` environment variable** (we will provide our own key when testing). Do not hardcode API keys.
...
- The Nebius Token Factory API is used for LLM calls
